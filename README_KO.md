...
| 명령어 세트 | 지원되는 비공식 NMOS opcode를 포함한 기본 6502 명령어 세트 | 지원되는 비공식 NMOS opcode를 제외한 기본 6502 명령어 세트 *비공식적인 65C02 전용 명령 (BRA, PHX, PHY, PLX, PLY, STZ, TRB, TSB, WAI, STP, Rockwell bit 연산)의 경우 현재 지원되지 않습니다.* |

`src/instruments_implementation.c` 에서 향후 지원 예정인 65C02 명령의 목록을 참조하십시오.
