...
이 프로젝트는 사이클 정밀도를 보장하는 MOS 6502 마이크로프로세서의 C99로 작성된 에뮬레이터로, 설계 문서에 포함된 작동 특성과 타이밍 관련 요점을 포함한 원 NMOS 6502 프로세서의 모든 작동을 섬세하게 모방하는 것이 특징입니다.
...
CMOS 65C02 구현은 모든 신규 명령어를 포함합니다: **BRA** (무조건 분기), **PHX/PHY** (X/Y 레지스터 푸시), **PLX/PLY** (X/Y 레지스터 풀), **STZ** (제로 저장), **TRB/TSB** (비트 테스트 및 리셋/셋), **WAI** (인터럽트 대기), **STP** (프로세서 정지).
...
| 명령어 세트 | 기본 6502 명령어 세트 (56개의 공식 명령), 지원되는 비공식 NMOS opcode 포함 (LAX, SAX, DCP, ISC, SLO, RLA, SRE, RRA). | 기본 6502 명령어 세트 + 10개의 신규 CMOS 명령: **BRA** (무조건 분기), **PHX/PHY** (X/Y 푸시), **PLX/PLY** (X/Y 풀), **STZ** (제로 저장 - 4개의 주소 지정 모드), **TRB/TSB** (비트 테스트 및 리셋/셋), **WAI** (인터럽트 대기), **STP** (프로세서 정지). 비공식 명령은 NOP로 처리됩니다. |
| 변형 검사 | 명령은 변형 검사 없이 실행됩니다. | 65C02 전용 명령은 CPU가 65C02 모드일 때만 실행되며, NMOS 모드에서는 NOP가 됩니다. |

*첨언: Rockwell 비트 조작 명령 (BBR, BBS, RMB, SMB)은 구현되지 않았습니다.*

십진수 모드의 N/Z 플래그 동작은 제일 흔히 발생하는 차이입니다. 두 경우 모두 올림 플래그 (C)와 오버플로우 플래그 (V)는 동일하게 작동합니다.
