...
A comprehensive test ROM for the MOS 6502 emulator that exercises various instructions and addressing modes. The test suite includes assembly source code (`test.asm`) with detailed comments, the compiled binary ROM file (`test.bin` at 131 bytes), a Python build script (`build_test_rom.py`), and a verification program (`verify_test.c`) for automated result checking.
